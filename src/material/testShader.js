...
void main() {
  vec2 u_resolution1 = vec2(1000.,930.);
    ...
    float amountLines = 500.;
    vec2 st = amountLines*vUv.xy/u_resolution1.xy;
    st.x *= u_resolution1.x/u_resolution1.y;

    // X: chia column, Y chia row 
    vec2 grid = vec2(20.0,100.);
    st *= grid;

    vec2 ipos = floor(st);  // integer
    vec2 fpos = fract(st);  // fraction

    vec2 vel = vec2(0.05*u_time*2.*max(grid.x,grid.y)); // time
    ...
    // Margins
    color *= step(0.1+random(1.0+ipos.y),fpos.y);
